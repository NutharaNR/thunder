...
var (
	...
	basicRecoveryUserSchema = testutils.UserSchema{
...
// BasicRecoveryFlowTestSuite tests the default email-link password recovery flow.
type BasicRecoveryFlowTestSuite struct {
	suite.Suite
...
func TestBasicRecoveryFlowTestSuite(t *testing.T) {
	suite.Run(t, new(BasicRecoveryFlowTestSuite))
}

func (ts *BasicRecoveryFlowTestSuite) SetupSuite() {
	...
	// Look up the bootstrap recovery and auth flow IDs
	recoveryFlowID, err := testutils.GetFlowIDByHandle("default-basic-flow", "RECOVERY")
	ts.Require().NoError(err, "Failed to get basic recovery flow ID")
...
func (ts *BasicRecoveryFlowTestSuite) TearDownSuite() {
...
func (ts *BasicRecoveryFlowTestSuite) TestBasicRecoveryFlow_Success() {
	...
	ok, err := testutils.AuthenticateWithPassword("username", ts.testUsername, ts.testPassword)
	ts.Require().NoError(err)
	ts.Require().False(ok, "Old password should be rejected after recovery")

	// Step 6: Verify new password works.
	ok, err = testutils.AuthenticateWithPassword("username", ts.testUsername, newPassword)
	ts.Require().NoError(err)
...
func (ts *BasicRecoveryFlowTestSuite) TestBasicRecoveryFlow_UnknownUsername() {
	ts.mockSMTP.ClearEmails()
...
func (ts *BasicRecoveryFlowTestSuite) TestBasicRecoveryFlow_InvalidToken() {
	ts.mockSMTP.ClearEmails()
...
func (ts *BasicRecoveryFlowTestSuite) TestBasicRecoveryFlow_RecoveryDisabledApp() {
