...
var (
	...
	smsRecoveryUserSchema = testutils.UserSchema{
...
func (ts *SMSOTPRecoveryFlowTestSuite) SetupSuite() {
	...
	// Look up the bootstrap SMS OTP recovery flow and auth flow IDs
	smsFlowID, err := testutils.GetFlowIDByHandle("default-password-recovery-sms-otp", "RECOVERY")
	ts.Require().NoError(err, "Failed to get SMS OTP recovery flow ID")
	ts.smsFlowID = smsFlowID

	// Patch the send_otp node in the flow to use the created sender ID.
	// Since the bootstrap flow has a placeholder senderId, we create a custom flow copy.
